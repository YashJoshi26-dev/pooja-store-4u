...
const categories = ["Electronics", "Mobiles", "Fashion", "Home & Kitchen", "Beauty", "Books", "Sports", "Toys", "Computers", "Accessories", "Furniture"];
...
export default function AddProduct() {
  ...
  // ✅ Actually saves to localStorage via productApi
    ...
    <div style={S.page}>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .ap-primary-btn:disabled { background: #ccc; cursor: not-allowed; transform: none; box-shadow: none; }
        ...
        .ap-save-btn {
          ...
          transition: all 0.2s; box-shadow: 0 4px 16px rgba(255,63,108,0.25);
        }
        ...
        <div style={S.topBarInner}>
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            <button
              ...
              style={{ background: "none", border: "none", cursor: "pointer", fontSize: 20, color: "#555", padding: "4px 8px" }}
            >
              ←
            </button>
            <span style={S.logo}><span style={{ color: "#ff3f6c" }}>Pooja</span>Store4u</span>
            <span style={{ color: "#ddd", fontSize: 18 }}>|</span>
            <span style={{ fontSize: 13, color: "#666", fontWeight: 500 }}>Add New Product</span>
          </div>
          <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
            <button
              onClick={() => navigate("/admin/products")}
              style={{ fontSize: 13, color: "#666", fontWeight: 600, cursor: "pointer", background: "none", border: "1.5px solid #e2e2e2", borderRadius: 8, padding: "7px 16px" }}
            >
              Cancel
            </button>
      ...
      <div style={{ background: "#fff", borderBottom: "1px solid #f0f0f0" }}>
        <div style={{ maxWidth: 1120, margin: "0 auto", padding: "10px 24px", display: "flex", alignItems: "center", gap: 6 }}>
          <span style={{ fontSize: 12, color: "#aaa", fontWeight: 500, cursor: "pointer" }} onClick={() => navigate("/admin/dashboard")}>Dashboard</span>
          <span style={{ color: "#ddd" }}>›</span>
          <span style={{ fontSize: 12, color: "#aaa", fontWeight: 500, cursor: "pointer" }} onClick={() => navigate("/admin/products")}>Products</span>
          <span style={{ color: "#ddd" }}>›</span>
      ...
      <div style={S.main}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 22 }}>
          <div>
            ...
            <p style={{ fontSize: 13, color: "#999" }}>Complete all steps to list your product on the shop</p>
          </div>
            ...
            return (
              <div key={s.label} style={{ display: "flex", alignItems: "center", flex: 1 }}>
                <div onClick={() => setActiveStep(num)}
                  ...
                  <span style={{ fontSize: 12, fontWeight: isActive ? 700 : 500, color: isActive ? "#ff3f6c" : isDone ? "#444" : "#bbb" }}>
                    {s.label}
                  </span>
                </div>
                {i < steps.length - 1 && (
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div className="ap-panel" style={S.card}>
                ...
                <div style={S.f}>
                  ...
                  <textarea className="ap-textarea" name="description" value={form.description} onChange={handleChange}
                    rows={4} placeholder="Describe the product — material, size, features..." />
            ...
            {activeStep === 2 && (
              <div className="ap-panel" style={S.card}>
                ...
                <div style={{ display: "flex", alignItems: "flex-start", gap: 8, background: "#fffbf0", border: "1px solid #ffe8a0", borderRadius: 8, padding: "10px 14px", marginBottom: 16 }}>
                  <span>💡</span>
                  <span style={{ fontSize: 12, color: "#7a6000" }}>Use white/plain backgrounds. Min 500×500px. Max 10MB each.</span>
                </div>
                <div
                  onDrop={handleDrop}
            ...
            {activeStep === 4 && (
              <div className="ap-panel" style={S.card}>
                ...
                <div style={S.f}>
                  ...
                  {form.status !== "active" && (
                    <p style={{ fontSize: 11, color: "#f59e0b", marginTop: 8, fontWeight: 600 }}>
                      ⚠️ Only "active" products appear on the shop.
                    </p>
                  )}
                </div>
                <hr style={S.hr} />
                ...
                <button className="ap-save-btn" onClick={handleSubmit} disabled={saving || saved}>
                  {saving ? "Publishing..." : saved ? "✓ Published! Redirecting..." : "Publish Product →"}
                </button>
                <p style={{ textAlign: "center", fontSize: 11, color: "#bbb", marginTop: 10 }}>
                  Active products appear instantly on the shop homepage
                </p>
              </div>
            )}
            ...
            <div style={{ display: "flex", justifyContent: "space-between", marginTop: 4 }}>
              {activeStep > 1
                ...
                : <div />}
              {activeStep < 4 && (
                <button className="ap-primary-btn" onClick={() => setActiveStep(s => s + 1)}>
                  Save & Continue →
                </button>
              )}
            </div>
      ...
      {saved && (
        <div className="ap-toast" style={{ position: "fixed", bottom: 24, right: 24, background: "#fff", border: "1px solid #e8e8e8", borderRadius: 12, padding: "14px 20px", display: "flex", alignItems: "center", gap: 12, boxShadow: "0 8px 32px rgba(0,0,0,0.12)", zIndex: 1000 }}>
          ...
          <div>
            ...
            <p style={{ fontSize: 12, color: "#888" }}>Now live on your shop. Redirecting...</p>
          </div>
        </div>
      )}
    </div>
