          ...
          {STEPS.map((s, i) => (
            <div key={s} style={{ display:"flex", alignItems:"center", flex:1 }}>
              <div className="flex items-center gap-2 cursor-pointer" onClick={() => i < step && setStep(i)}>
                <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold transition-all ${i < step ? "bg-green-500 text-white" : i === step ? "bg-slate-900 text-white" : "bg-gray-200 text-gray-400"}`}>
                  {i < step ? "✓" : i + 1}
                </div>
                <span className={`text-sm font-semibold hidden md:block ${i === step ? "text-slate-900" : "text-gray-400"}`}>{s}</span>
              </div>
              {i < STEPS.length - 1 && <div className={`h-0.5 flex-1 mx-3 rounded ${i < step ? "bg-green-500" : "bg-gray-200"}`}/>}
            </div>
          ))}
        </div>
        ...
        <div className="grid lg:grid-cols-3 gap-8">
                ...
                <motion.div key="cart" initial={{opacity:0,x:20}} animate={{opacity:1,x:0}} exit={{opacity:0,x:-20}}
                  ...
                  <div className="space-y-4">
                    {cart.map(item => {
                      ...
                      return (
                        <div key={id} className="flex gap-4 p-3 bg-gray-50 rounded-xl">
                            ...
                            <div className="flex items-center gap-2 mt-2">
                              <div className="flex items-center gap-1 bg-white border border-gray-200 rounded-lg p-0.5">
                                <button onClick={() => updateQuantity(id, item.quantity - 1)}
                                  className="w-6 h-6 rounded flex items-center justify-center text-gray-600 hover:bg-red-50 hover:text-red-500 transition font-bold text-sm">−</button>
                                <span className="w-6 text-center text-sm font-bold">{item.quantity}</span>
                                <button onClick={() => updateQuantity(id, item.quantity + 1)}
                                  className="w-6 h-6 rounded flex items-center justify-center text-gray-600 hover:bg-green-50 hover:text-green-600 transition font-bold text-sm">+</button>
                              </div>
                              {/* ✅ Remove button */}
                          ...
                          <p className="font-bold text-slate-900 flex-shrink-0 self-start">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    {[
                      ...
                      <div key={f.name} className={f.col === 2 ? "col-span-2" : ""}>
                        ...
                        <input name={f.name} value={delivery[f.name]} onChange={handleDeliveryChange} placeholder={f.placeholder}
              ...
              {step === 2 && (
                <motion.div key="payment" initial={{opacity:0,x:20}} animate={{opacity:1,x:0}} exit={{opacity:0,x:-20}}
                  ...
                  <div className="flex gap-3 mb-6">
                    {[{id:"upi",label:"UPI",icon:"📱"},{id:"card",label:"Card",icon:"💳"},{id:"cod",label:"Cash on Delivery",icon:"💵"}].map(m => (
                  ...
                  {payment.method === "upi" && (
                    <div>
                      ...
                      <input name="upiId" value={payment.upiId} onChange={handlePaymentChange} placeholder="rahul@upi"
                        className="w-full border border-gray-200 rounded-lg px-3 py-2.5 text-sm outline-none focus:border-slate-400 transition"/>
                    </div>
                  )}
                  {payment.method === "card" && (
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 self-start sticky top-24">
            ...
            <div className="border-t border-gray-100 pt-4 space-y-2">
              ...
              {shipping === 0 && (
                <p className="text-xs text-green-600 font-medium bg-green-50 px-2 py-1 rounded-lg">
